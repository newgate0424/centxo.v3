...
export function DatePickerWithRange({
    ...
    React.useEffect(() => {
        ...
        // Special check for "This Month" - dashboard may modify end date to today
        const today = new Date();
        const isThisMonthRange =
            date.from && date.to &&
            isSameDay(date.from, startOfMonth(new Date())) &&
            (isSameDay(date.to, endOfMonth(new Date())) || isSameDay(date.to, today));

        if (isThisMonthRange) {
            setSelectedPreset(t.common.thisMonth);
            return;
        }

        const foundPreset = presets.find(p => {
            const pDate = p.getValue();
            // Check if dates match (ignoring time components for start/end comparison safely if already normalized, but isSameDay is safest)
                    ...
                    <div className="flex">
                        ...
                        <div className="p-2">
                            ...
                            <div className="flex justify-end gap-2 mt-2">
                                ...
                                <Button onClick={() => setIsOpen(false)}>{t.common.apply}</Button>
                            </div>
                        </div>
                    </div>
                </PopoverContent>
