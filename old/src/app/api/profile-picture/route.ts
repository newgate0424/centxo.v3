...
// ⚠️ NOTE: This API is currently NOT USED in the application
// Consider removing if not needed, or implementing it in message/chat UI
...
const CACHE_DURATION = 6 * 60 * 60 * 1000; // 6 hours (increased from 1 hour)
...
const TOKEN_CACHE_DURATION = 2 * 60 * 60 * 1000; // 2 hours (increased from 30 minutes)

async function getPageToken(pageId: string, userAccessToken: string): Promise<string | null> {
    // Try Redis cache first
    const cacheKey = `profile_pic:page_token:${pageId}`;
    const cached = await getCached<string>(cacheKey);
    if (cached) {
        console.log('[getPageToken] Returning from Redis cache');
        return cached;
    }
    
    // Fallback to in-memory cache
    const memCached = pageTokenCache.get(pageId);
    if (memCached && Date.now() - memCached.timestamp < TOKEN_CACHE_DURATION) {
        console.log('[getPageToken] Returning from memory cache');
    ...
    try {
        ...
        if (pageData.access_token) {
            // Cache in both Redis (2 hours) and memory
            await setCache(cacheKey, pageData.access_token, 2 * 60 * 60);
            pageTokenCache.set(pageId, {
                token: pageData.access_token,
                timestamp: Date.now()
            });
            console.log('[getPageToken] Fetched and cached new token');
